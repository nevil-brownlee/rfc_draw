...
class rdglob:  # Global variables for rfc-draw's objects
    ...
    def __init__(self, parent, root, m_text):
        ...
        #self.f_font = "DroidSansMono"  # This works, above version doesn't <<<
    ...
    # Click b3 to edit a text object
        ...
        #self.root.bind("<Control-Key-g>", self.new_group)
    ...
    def bind_keys(self):
        ...
        self.root.bind('<KeyPress-c>',self.on_key_press) # copy
        self.root.bind('<KeyPress-a>',self.on_key_press) # arrows
        self.root.bind('<KeyPress-n>',self.on_key_press) # no arrows
        self.root.bind('<KeyPress-e>',self.on_key_press) # syn ends
        self.root.bind('<KeyPress-b>',self.on_key_press) # bare ends
        self.root.bind('<KeyPress-f>',self.on_key_press) # flip
        self.root.bind('<KeyPress-r>',self.on_key_press) # reverse
        self.root.bind("<KeyPress-equal>", self.on_key_press)
        self.root.bind('<KeyPress-u>',self.on_key_press) # ungroup
    ...
    class n_rect:  # Rectangle with white fill and centred text
        def __init__(self, rdg, drawing, r_coords, r_text): # n_rect, with text
    ...
    def get_object(self, item_ix):
        ...
        if item_ix in self.objects.keys():  # It's a known object
            ...
            print("Unknown object, item_ix %d, tk type %s %s" % (
                item_ix, item_type))
            return None
    ...
    def restore_rect(self, r_coords, r_text, g_nbr):
        rdo = self.n_rect(self.rdg, self.drawing, r_coords, r_text)
        print("restore_rect: ", end="");  rdo.print_n_rect()
        if True:
            self.objects[rdo.rect_id] = self.object(rdo.rect_id,
                rdo, "n_rect", r_coords, r_text, 0, 0)  # n_rect obj
            self.objects[rdo.text_id] = self.object(rdo.text_id,
                rdo.text_id, "text", r_coords, r_text, 0, rdo.rect_id) 
    ...
    def new_group_nbr(self):
        ...
        if self.rdg.n_groups == self.rdg.mx_groups:
            print(">>> Can only have at most %d groups!" % \
                self.rdg.mx_groups)
            exit()
        else:
            return g_nbr
    ...
    def read_from_rdd(self, fn):
        if not os.path.exists(fn):
            print("No file %s, will write it on closing" % fn)
            self.f_width = 10.55;  self.f_height = 17
            #  10.55 px width, 17 px height work well for TkFixedFont !
        else:
            f = open(fn, "r")
            for line in f:
    ...
    def obj_to_save_str(self, val):
        ...
        if d_type:
            ...
            print(">>> obj_to_save_str, unknown object type <<<")
            exit()
        self.dump_objects("read_from_rdd")
    ...
    def save_to_rdd(self, save_file_name):  # Write rfc-draw data (.rdd) file
        ...
        s_file.write("mono_font width 10.55 height 17 pixels\n")
    ...
    def copy_g_member(self, ncgo, old_gm):
        ...
        if isinstance(n_obj_key, self.n_rect):  #$ it is an n_rect
            ...
            elif obj.o_type == "line":                 # rdg is self here!
                print("  g_m edr_coords: %s" % edr_coords)
                print("copy_g_member:  line %s" % obj)            
        """
        else:
            if obj.o_type == "line":                 # rdg is self here!
                print("copy_g_member:  line %s" % obj)
            elif obj.o_type == "text":
                print("<>copy_g_member: g_tk_id = %d" % n_gm.g_tk_id)
            else:
                print("restore_??? %s" % obj)

                nmo = self.new_g_member_object(ncgo, old_gm)
                print("= back from new_g_member, nmp =, n_obj_key %s" % nmo)
                n_gm = self.g_member(self.rdg, ncgo.object.g_nbr,
                                     n_obj_key, n_obj_key, obj, old_gm.rel_coords)
                print("**copy_g_member: n_gm = %s" % n_gm)
                edr_coords = self.screen_coords(n_gm.g_object.i_coords,
                                                old_gm.rel_coords)
                print("  g_m edr_coords: %s" % edr_coords)
        ...
        if obj.o_type == "text":
            ...
            for ogm in obj.object.g_members:  # Members of original object
                ...
                if o_key in ncgo.object.g_members:
                    print("   XXX obj already in g_members XXX")
                else:
                    print("   ... ncgo >%s<" % ncgo)  ## ncgo key 4, group 2
                    print("   ''' g_object /%s/" %  obj)
    ...
    def on_key_press(self, event):
        ...
        if key == "c":  # Copy a tk object
            self.copy_object(self.current_object)
             ...
             if self.last_button != "group":
                print(">> Must be in Group mode to ungroup object")
                return
             self.drawing.delete(self.current_object.object.g_rect_id)
                 # Undraw group's edr (don't try to re-use it's group nbr!
             self.objects.pop(self.current_object.key)
        else:
            if self.last_button != "line":
                print(">> Must be in Line mode to flip or reverse object")
                return
    ...
    def on_b3_click(self, event):  # b3 (Right button) to edit a text object
        ...
        if item:
            ...
            if item_type != "text":        
                print("Your b3 click was not on a text object!")
            else:
                print("b3_click: obj = %s" % obj)
                #co self.current_object = self.objects[item_x]
    ...
    def delete_object(self, obj):
        ...
        po = self.objects.pop(obj.key)  # Remove object from dictionary
        ds = self.obj_to_save_str(obj)
        self.deleted_objects.append(ds)
    ...
    def on_delete_key(self, event):  # Delete key pressed
        ...
        item_ix, obj = self.rdg_closest(mx,my)
        if obj:
    ...
    def on_insert_key(self, event):  # Insert key pressed
        if len(self.deleted_objects) == 0:
            print("\a")  # BEL
            self.display_msg("No deleted objects", 'normal')
        else:
            d_obj = self.deleted_objects.pop()
            print("insert_key: d_obj = %s" % d_obj)
            if "group " in d_obj:
                print("Inserting a group object <<<<<<<")
            print("Insert: ds = >%s<" % d_obj)
            self.restore_object('99 '+d_obj)  # ds starts with an obj id
    ...
    def mouse_action(self, event):
        if self.double_click_flag:
            ...
            if self.ln_mode == "group":
                print("Can't edit a Text in 'group' mode!")
            else:
