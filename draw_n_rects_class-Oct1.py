...
class draw_n_rects:  #  Rectangle with white fill and centred text
    ...
    def __init__(self, parent, root, rdg):
        ...
        self.rect_id = None
        self.parent_id = 0;  self.v1 = self.v2 = None
    ...
    def restore_object(self, r_coords, r_text, parent_id, v1, v2):
        ...
        print("/1/ current_object %s" % self.rdg.current_object)
    ...
    def mk_save_str(self, val):  # Make object's rdd file entry
        ...
        return "(%s %d) %s \"%s\" %s %s %s" % ("n_rect", val.key, \
            coords, str, 0, val.v1, val.v2)
            # Corrected 20 Sep 2024: n_rects have no parent_id 
    ...
    def delete(self):
        ...
        all_items = self.rdg.drawing.find_all()  # tuple if tk ids
        for id in all_items:
            id_type = self.rdg.drawing.itemcget(id, type)
            print("5d, %s" % id, id_type)
    ...
    def nr_closest(self, mx,my):
        ...
        if len(item) == 0:  # Empty tuple
            ...
            return None, None
        print("=== nr_closest %d,%d" % (mx,my))
        """
        all_items = self.rdg.drawing.find_all()  # tuple if tk ids
        for id in all_items:
            print("id = %d" % id)
            item = self.rdg.drawing(id)
            print("5d, %s >%s<, mx,my %d,%d" % (id, item, mx,my))
        print("+++++++++++++")
        """
        item_x = item[0]
        ...
        #item_type = self.drawing.itemcget(item_x, type)
        #item_coords = self.drawing.itemcget(item_x, bbox)
        #print("*** nr_closest(): item_x = %s (%s), [%s], mx,my %d,%d" % (
        #    item, item_type, item_coords, mx,my))

        j = 0
        for key in self.rdg.objects:
            val = self.rdg.objects[key]
            j += 1
            #print("!@!@ j %d, val %s" % (j, val))
            ds = self.rdg.get_save_string(val)
            orc = val.a_obj.i_coords
            cx = int(orc[0]+orc[2]/2)
            cy = int(orc[3]+orc[3]/2)
            dist = abs(cx-mx) + abs(cy-my)
            print("@@2: j %d, dist %d, ds %s" % (j, dist, ds))
    ...
    def b1_dot(self, mx,my):
        # Draw dot at top-left of closest (tk) object **before this b1_click**
        #print("b1_dot: %d,%d, obj %s" % (mx,my, obj))
        #nrc = self.rdg.hd_colours[obj.a_obj.rect_id % 8]
        #abb = obj.a_obj.bbox()
        #x0=abb[0]; y0=abb[1];  x1=abb[2];  y1=abb[3]
        self.drawing.create_oval(mx-3,my-3,mx+3,my+3,fill='black',width=1)
    ...
    def nr_b1_click(self, event):  # B1 (left button) to select an object
        ...
        if not item:  # Empty tuple, nothing drawn yet
            ...
            if obj.o_type == "n_rect":
                self.b1_dot(mx,my)  ## draws black dot to start **new** rectangle
                
                abb = obj.a_obj.bbox()
                x0=abb[0]; y0=abb[1];  x1=abb[2];  y1=abb[3]
                print("b1_click: %d/%d abb [%d,%d, %d,%d]" % (mx,my, x0,y0, x1,y1))
                print("item_ix %d: " % item_ix, end="")
                obj.a_obj.print_n_rect()
                self.rdg.drawing.itemconfigure(obj.key, fill="pink")
                ### makes **previous** rectangle pink
                ###    so **previous** was closest to cursor position
                
                self.rdg.current_object = obj  ###################
                print("/2/ current_object %s" % self.rdg.current_object)

                print("!.!.! rect_id = %d" % obj.a_obj.rect_id)
                self.rdg.region = self.rdg.where(
                    self.rdg.current_object.a_obj, mx,my)
                print(">1> %d,%d: current_obj %s, self.rdg.region = %d" % (
                    mx,my, self.rdg.current_object, self.rdg.region))
            ...
            if obj.o_type == "n_rect":
                ...
                self.rdg.region = self.rdg.where(
                    self.rdg.current_object.a_obj, mx,my)
                print("current_obj = %s, region = %d" % (obj, self.rdg.region))
                ...
                self.rdg.current_object = obj
                print("/4/ current_object %s" % self.rdg.current_object)

                print("==> Can't move a '%s' in '%s' mode" % (
