...
import os.path, re, sys, time, datetime
...
import arrow_lines_class_26 as alc   # Draw lines with direction arrows
import draw_lines_class_26 as dlc    # Handles line objects
import draw_n_rects_class_26 as drc  # Handles n_rect objects
import draw_headers_class_26 as dhc  # Handles rfc-draw headers
...
class rdglob:  # Global variables for rfc-draw's objects
    ...
    def __init__(self, parent, root, m_text):
        ...
        self.rdg = self
        print(">>> rdg startup, root >%s<, self.rdg >%s<" % (root,self.rdg))
        ...
        print("@@@ self.f_font %s" % self.f_font)
        ...
        self.m_text.tag_config('warning', foreground="deepskyblue2")
    ...
    # Click b3 to edit a text object
        ...
        print("self.rdg.save_file_name >%s<" %self.rdg.save_file_name)
        self.dhc_tool = dhc.draw_headers(self.drawing, self.root, self.rdg)
        print("rdgc, self.dhc_tool %s" % self.dhc_tool)
    ...
    def display_msg(self, text, tag):  # Display text in Message area
        ...
        #print("display_msg, text %s, tag %s" % (text, tag))
        self.m_text.delete('1.0', tk.END)
        self.m_text.insert('1.0', text, tag)
        if tag == "error" or tag == "warning":
            playsound("BSB-counter_bell.wav")
    ...
    def read_from_rdd(self, fn):
        print(">>> starting read_from_rdd")
        self.test_string = fn
        self.save_file_name = fn
        print("+++ read_from_rdd: new_drawing, fn >%s<" %
              self.save_file_name)
        last_mode = 'rect'
        
        if not os.path.isfile(fn):  # No save_file
            self.new_drawing = True
            print("=-=-=-= rdg: new_drawing True")
            ##self.display_msg(self, "New drawing, will write %s on closing" % \
            #    fn, "normal")  # No bell sound
        else:
        ...
        print("rgdc: new_drawing %s" % self.new_drawing)
        f = open(fn, "r")
        for line in f:
            ds = line.rstrip('\n')
            ...
            if ds.find("root_geometry") >= 0:
                la = ds.split(" ")
                print("7890: la %s" % la)
                print("$$ la >%s<, self.f_font %s, self.rdg >%s<" % (
                    la, self.f_font, rdglob.rdg))
        ...
        print("leaving read_from_rdd, last_mode %s, self.new_drawing %s" % (
            last_mode, self.new_drawing))
        return last_mode
