...
class draw_groups:  # rectangle+text objects for rfc-draw
    ...
    def set_event_handlers(self):
        ...
        self.drawing.bind_class('Canvas','<Double-1>', self.dg_b1_double_action)
        ...
        ##self.root.bind('<Key>',  self.on_key_click)
    ...
    def edr_to_group(self):  # Creates group from edr
            ...
            if t[0].startswith("edr"):
                ...
                for obj in group_objs:  # Add enclosed objects to group
                    ...
                    if obj.o_type == "n_rect":
                        ...
                        print("e_t_g: bad object type, %s" % obj)
        ...
        print("edr now: ", end="");  self.print_group("", eo)
        print("= = = = =")
        if not found_edr_tag:
            print(">> The object you b3-double-clicked on is not an edr")

        ##gx0,gy0, gx1,gy1 = edr.object.bbox()
        ##g_coords = [gx0,gy0, gx1,gy1]
        ##go = self.new_group(eo, g_coords)  # go is a group object
        ###print("??? go = %s" % go)

        ##print("eo now >%s<" % eo)
        ##self.rdg.drawing.delete(eo.g_rect_id)  # Delete template,
                                               #   group remains on-screen
        
        self.rdg.dump_objects("After edr_to_group")
        ##exit()
    ...
    def dg_b1_click(self, event):
        self.drawing.after(250, self.dg_b1_double_action, event)
    ...
    def dg_b1_action(self, event):  # B1 (left button) to select an object
        ...
        if not item:  # Empty tuple, nothing drawn yet
            ...
            if obj.o_type == "group":
                ...
                if not obj:
                    print("No surrounding group found!")
                    exit()
    ...
    def dg_b1_double_action(self, event):
        if self.double_click_flag:
            ...
            self.double_click_flag = False
            print("b1_double not implemented yet <<<<<<<")
        else:
    ...
    def dg_b3_action(self, event):
        if self.double_click_flag:
            ...
            self.double_click_flag = False
            self.edr_to_group()
            #dgr.draw_groups.where_am_i(self)

        else:  # B3 (right button) to edit a Text
            if self.rdg.ln_mode == "group":
                print(">> Can't edit Text using b3 in group mode")
            else:
