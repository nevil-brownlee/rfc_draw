...
class draw_headers:  # pkt header objects for rfc-draw
    ...
    class header:  # Creates a new Header object
        ...
        #     Their move function must move all their contained objects
            ...
            #print("=========== header %d, nothing drawn ==========")
            #print("header %d,%d, %d, %d" % (self.x0, self.y0, self.x1, self.y1))
            #self.rdg.time_now("hdr, nothing drawn")
            ...
            #print(">< header >< self.rdg.new_drawing %s, %d rows" % (
    ...
    class row:
        def __init__(self, drawing, rdg, h, r_lines):
            ...
            self.r_mx_txt_lines = 0  # max nbr of text lines in fields
        ...
        def change_row_lines(self, r_rdo, rl_delta):
            ...
            if rl_delta > 0:  # Down (add a line)
                ...
                if r_clo.r_lines == 1:
                    ...
                    return
                elif r_clo.r_lines-1 < r_clo.r_mx_txt_lines:
                    self.rdg.display_msg(
                        "Row contains field(s) with %d lines" %
                        r_clo.r_mx_txt_lines, "error")
    ...
    class field:
        def __init__(self, rdg, h, r, text, f_col, width, fcolour):
            ...
            if n_txt_lines > self.r.r_mx_txt_lines:
                 self.r.r_mx_txt_lines = n_txt_lines
        ...
        if self.double_click_flag:
            ...
            if not item_id:  # Empty tuple, nothing drawn yet
                ...
                if d_type == "[line]":  # Is it a tic?
                    ...
                    elif f_id:  # It's a bar
                        #print("It's a bar, f_id %d <><>" % f_id)
                        f_rdo = self.rdg.objects[item_id]
                        #print("f_rdo = %s" %f_rdo)
