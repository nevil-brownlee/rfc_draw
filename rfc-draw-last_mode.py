...
import draw_headers_class_28 as dhc  # Handles pkt header objects
...
##print("RD: rdg.f_height %s" % rdg.f_height)
...
if last_mode:
    previous = last_mode
print("save_file %s, previous = %s" % (save_file_name, previous))

if previous == "rect":  # Set startup mode
    drc_tool.set_event_handlers();  rdg.set_mode('rect')
elif previous == "line":
    dlc_tool.set_event_handlers();  rdg.set_mode('line')
if previous == "text":
    drc_tool.set_event_handlers();  rdg.set_mode('text')
if previous == "header":
    drc_tool.set_event_handlers();  rdg.set_mode('header')
