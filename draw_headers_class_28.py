...
import rfc_draw_globals_class as rdgc  # rfc-draw globals
import draw_texts_class as dtc    # Handles text objects

#import traceback  # For debugging using n_stack (below)
    
class draw_headers:  # pkt header objects for rfc-draw
    ...
    class header:  # Creates a new Header object
        def __init__(self, drawing, root, rdg, h_nbr, h_coords):
        ...
        #     Their move function must move all their contained objects
            ...
            print("$$$ tl_coords %s" % self.tl_coords)
            self.hdr_id = self.rdg.add_to_layer(2, # Draw header top line
                self.drawing.create_line, self.tl_coords, fill="red")
            # We need a hdr_id for each header in our drawing!
            print("=========== header %d, hdr_id %d, top line drawn ==========",
                  (self.h_nbr, self.hdr_id))
            print("header %d,%d, %d,%d" % (self.x0, self.y0, self.x1, self.y1))
            self.rdg.time_now("hdr, top line drawn")
            ...
            h_rdo = self.rdg.object(  # hdr obj just has it's top line
                ...
                0, self.h_nbr, 0)  # Parent for rows
            ##print("-->> hdr_obj key %d" % self.hdr_id)
            ...
            print("?? ?? hdr_id %d, objects[%d] >%s<" % (
                self.hdr_id, self.hdr_id, self.rdg.objects[self.hdr_id]))
            self.rdg.dump_objects("header %d drawn, hdr_id %d" % (
                              self.h_nbr, self.hdr_id))
            
            #draw_headers.wait_for_input(self, "drawn top line")
            print("5678: self.rdg >%s<" % self.rdg) ### prints rdglob object ...
            self.rdg.time_now(msg)                   #  works
            if self.rdg.new_drawing and len(self.rows) == 0:  ### fails, 
                   # Didn't read drawing from save file
                ...
                print("<><> header.new h %s" % self)
                #print("header: hr %s" % self.hr)
                #draw_headers.wait_for_input(self, "drawn hdr, but NOT col_nbrs")
                self.draw_col_nbrs(self, self.hr)  # i.e. h, hr
                self.rdg.time_now("hdr, col_nbrs drawn")
                #self.rdg.dump_objects("col_nbrs drawn, %d rows" % len(
                #    self.rows))
            #print("new header, h_nbr %d, %s" % (self.h_nbr, self))
            draw_headers.headers.append(self)
            #print("HHH new_drawing: draw_headers.headers len %d, >%s<" % (
            #    len(draw_headers.headers), draw_headers.headers))
        ...
        def mk_save_str(self, val):  # Make header's rdd file entry
            ...
            h = val.a_obj;  d_id = val.key
            #print("$ $ $ header mk_save_str: self.h_nbr %d" % h.h_nbr)
            d_type = "header"
            x0,y0, x1,y1 = h.bbox()
            ...
            ds = "(%s %d) %s \"%s\" %s %s %s" % (
                "header", val.key, coords, "H", 0, h.h_nbr, 0)
        ...
        def restore_object(self, h_coords, h_text, parent_id, v1, v2):  # For rdgc.on_insert_key()
            ...
            draw_headers.headers.append(h_obj)
    ...
    class row:
        def __init__(self, drawing, rdg, h, r_lines):
            ...
            #print("class row, r_lines %d, vbl_len_row %s" % (
            #    self.r_lines, self.vbl_len_row))
            ...
            #print("row header %d, x0,y0, x1,y1 %d,%d, %d,%d" % (
            #    self.h.h_nbr, self.x0,self.y0, self.x1,self.y1))
        ...
        def clicked_on_tic(self, hn, rn, cn):  # Clicked on a tic (in class row)
            ...
            #print("@@@ c_o_t: draw_headers.headers %s" % draw_headers.headers)
            self.h_clo = draw_headers.headers[hn-1]  # header 1 has index 0
            self.r_clo = self.h_clo.rows[rn-1]  # r_clo is a class object
            #print("   header %d, row %d" % (self.h_clo.h_nbr, self.r_clo.r_nbr))
    ...
    class field:
        def __init__(self, rdg, h, r, text, f_col, width, fcolour):
            ...
            self.row_id = self.r.row_id;  self.r_nbr = self.r.r_nbr
            #print("||||||||| self.row_id %d (%s)" % (  # << correct here
            #    self.row_id, type(self.row_id)))  ##;  exit()
            ...
            self.text_obj = self.rdg.dtc_tool.restore_object(
                (self.f_cx, self.f_cy), text, self.row_id,
        ...
        def move_bar(self, f_obj, fx_delta):
            ...
            #print("f_clo  >%s<" % f_clo)
            #print("rf_clo >%s<" % rf_clo)
            #print("rf_clo == f_clo %s" % rf_clo == f_clo)
            #print("?=?=? rf_rdo >%s< fx_delta %s" % (rf_rdo, fx_delta))
            if fx_delta > 0:  # Move bar right
                if rf_clo.width == 1:
                    ...
                    lf_clo, rf_clo = draw_headers.field.change_fieldsdisplay(
                        self, lf_clo, rf_clo)
            else:  # Move bar left  <<<<<<<<<<<
                if lf_clo.width == 1:
        ...
        def split_field(self, fs_rdo, at_col):  # fs = "field to split"
            ...
            #    fs_clo.f_col, fs_clo.width, fs_clo.row_id))
            ...
            new_lf_width = at_col+1 - new_lf_f_col       #  8, 12 << Correct
            ...
            lf_obj.a_obj.parent_id = lf_clo.row_id
            ...
            rf_obj.a_obj.parent_id = rf_clo.row_id
        ...
        def redraw(self, f_rdo):
            ...
            self.rdg.drawing.itemconfigure(
                f_rdo.text_id, state=tk.NORMAL)
            if self.f_col != 0:  # Redraw bar at left
                self.rdg.drawing.itemconfigure(
                    self.bar_id, state=tk.HIDDEN)

        def __str__(self):
            return "class field: self.h %s, self.r %s, self.text %s" % (
        ...
        def mk_save_str(self, val):  # Make field's rdd file entry
            ...
            #print("mk_s_s: f_clo %s, type %s" % (f_clo, type(f_clo)))
            #print("@|@|@ i_coords %d,%d, %d,%d" % val.i_coords)
            fcx0,fcy0, fcx1,fcy1 = f_clo.bbox()
            #print("[.1 f_clo bbox %d,%d, %d,%d" % (fcx0,fcy0, fcx1,fcy1))
            #ri = float(val.i_coords[2]);  print("+++ ri %s" % ri)
            #ii = int(ri);  print("+++ ii %s" % ii)
            
            i_coords = f_clo.bbox();  coords = []
            for c in i_coords:
                coords.append(int(float(c)))
            #print("field's coords >%s<" % coords)
            sit = str(val.i_text)  # rdg.edit_esc_key updates i_text
            sit = sit.replace('"', '\\"')  # Escape \" chars
            sit = sit.replace("\n", "\\n")   # Escape \n chars
            #print("mks field: (%s %d) %s \"%s\" %s %s %s" % (
            #    "field", val.key, coords,
            #    sit, f_clo.row_id, f_clo.f_col, f_clo.width))
    ...
    def restore_object(self, obj_type,  # For read_from_rdd()
        ...
        if obj_type == "header":
            ...
            print("=== f >%s<" % f)
    """       
    def rd_where(self, m_clo, mx,my):  # Simple where for h/r/f items
        res_px = self.rdg.res_px
        print("\n\n@ rd_where: m_clo %s (%s), r_nbr %d, " % (
            m_clo, type(m_clo), m_clo.r_nbr))
        
        print("          coords %d,%d, %d,%d,  mx,my %d,%d, res_px %d" % (
            m_clo.x0, m_clo.y0, m_clo.x1, m_clo.y1, mx,my, res_px))
        if mx < m_clo.x0 or mx > m_clo.x1:
            print("mx,my %d,%d: rd_where = far" % (mx,my))
            return self.rdg.far  # Outside clo's width
        elif my <= m_clo.y0-res_px:  # Above clo
            print("mx,my %d,%d: rd_where = top" % (mx,my))
            return self.rdg.top
        elif my > m_clo.y1:  # Below clo
            if ((my > m_clo.y0 and my-m_clo.y0 <= res_px) and  # bottom
                    (mx >= m_clo.x0 and mx-m_clo.x0 <= res_px)):  # left
                print("self.rdg.ll = %d" % self.rdg.ll)
                return self.rdg.ll
            if (my-m_clo.y1) <= res_px:
                print("mx,my %d,%d: rd_where = bottom" % (mx,my))
                return self.rdg.bot

            if (mx > m_clo.x0 and mx-m_clo.x0 <= res_px):  # Near left edge
                print("Near left edge")
                if (my < m_clo.y0 and my-m_clo.y0 <= res_px): # Near bottom
                    print("@@@ ll")
                    return self.rdg.ll
            elif (mx < m_clo.x1 and m_clo.x1-mx <= res_px):  #  Near right edge
                if (my > m_clo.y0 and my-m_clo.y0 <= res_px): # Near bottom
                    print("@@@ lr")
                    return self.rdg.lr      
            elif my >= m_clo.y1:  # Below rdo
                if (my-m_clo.y1) <= 20:
                    print("mx,my %d,%d: rd_where = bottom" % (mx,my))
                    return self.rdg.bot
                else:
                ...
                print("mx,my %d,%d: rd_where = middle" % (mx,my))
                return self.rdg.middle
    ...
    def dg_b1_action(self, event):  # B1 (left button) to select an object
        ...
        if self.double_click_flag:
            ...
            print("12345: draw_headers.rdg  >%s<" % draw_headers.rdg)
            mx, my = (event.x, event.y)  # Mouse position
            self.rdg.last_mx = mx;  self.rdg.last_my = my  # Last mouse position
            ...
            #print("D D D b1_action: crl_coords %s, mode %s" % (
            ...
            if not item_id:  # Empty tuple, nothing drawn yet
                ...
                #self.rdg.dump_objects("Header drawn!")
            else:
                item_type = self.drawing.type(item_id)  # To get tk item id's
                item_coords = self.drawing.bbox(item_id)  # tuple
                ...
                elif d_type == "field":  # Top row to move whole header h_1
