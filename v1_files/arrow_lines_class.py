...
class a_line:  # Draw line with arrows showing direction
    def __init__(self, drawing, coords, rdg, parent_id, v1, v2):
    ...
    def mk_save_str(self):  # Make object's rdd file entry
        print("**** a_line: mk_save_str, self >%s<" % self)
        ##self.rdg.dump_objects("starting mk_save_str")
        str = ""
        str += "a" if self.arrowheads else "n"  # None
        str += "e" if self.syntax_end_mark else "b"  # Bare
        return "(%s %d) %s \"%s\" %s %s %s" % ("line", self.lbd_id,
    ...
    def restore_object(self, l_coords, l_text, parent_id, v1, v2):
        ...
        print("ALC.restore_line: l_coords %s, l_text >%s<" % (l_coords, l_text))
        a_line = alc.a_line(self.drawing, l_coords, self.rdg)
        print("<><><> a_line %s" % a_line)
        ...
        line_obj = self.object(
            self.lbd_id, a_line, "line", l_coords, l_text, parent_id, 0, 0)
